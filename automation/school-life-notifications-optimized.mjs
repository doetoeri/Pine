...
const PERSONAL_MATERIAL_LIMIT = 160;
const STATE_GET_BATCH = 100;
...
async function queryRowsByIds(collectionRef, ids = []) {
  const uniqueIds = [...new Set((Array.isArray(ids) ? ids : []).map((id) => String(id || "")).filter(Boolean))];
  if (!uniqueIds.length) return [];
  const rows = [];
  for (let index = 0; index < uniqueIds.length; index += STATE_GET_BATCH) {
    const refs = uniqueIds.slice(index, index + STATE_GET_BATCH).map((id) => collectionRef.doc(id));
    const snapshots = await collectionRef.firestore.getAll(...refs);
...
async function loadUserContext(root, user, context) {
  const userRef = root.collection("schoolLifeUsers").doc(user.uid);
  const [preferenceSnapshot, recurringMaterials, datedMaterials] = await Promise.all([
    userRef.collection("settings").doc("notifications").get(),
    queryRows(userRef.collection("materials").where("date", "==", "").limit(100)),
    queryRows(userRef.collection("materials").where("date", ">=", context.date).where("date", "<=", context.tomorrow).limit(PERSONAL_MATERIAL_LIMIT)),
  ]);
  const personalMaterials = active(uniqueRows([...recurringMaterials, ...datedMaterials]));
  const seed = {
    user,
  ...
  const relevantBundles = [
    ...bundlesFor(context, seed, context.date),
    ...bundlesFor(context, seed, context.tomorrow),
  ];
  const materialStateIds = relevantBundles.flatMap((bundle) => bundle.materials.map((item) => item.stateKey).filter(Boolean));
  const assessmentStateIds = relevantBundles.flatMap((bundle) => bundle.assessments.map((item) => item.id).filter(Boolean));
  const [materialStates, assessmentStates] = await Promise.all([
    queryRowsByIds(userRef.collection("materialStates"), materialStateIds),
    queryRowsByIds(userRef.collection("assessmentStates"), assessmentStateIds),
  ]);
  return {
    ...seed,
...
function lazySubscriptionMap(root) {
  let promise = null;
  return {
    async get(uid) {
      if (!promise) promise = subscriptionMap(root);
      return (await promise).get(uid) || [];
    },
  };
}
...
async function sendToUser({ root, messaging, subscriptionsByUid, user, type, title, body, date, period = 0, relatedId = "", dedupeKey, urgency = "normal" }) {
  ...
  const subscriptions = await subscriptionsByUid.get(user.uid);
  let sent = 0;
...
async function dispatchMovingClasses({ root, messaging, subscriptionsByUid, context, userContext, now }) {
  ...
  for (const bundle of bundles) {
    ...
    const targetMs = dateClockMs(lesson.date, lesson.startTime) - prefs.movingClassMinutes * 60_000;
...
async function dispatchAssessments({ root, messaging, subscriptionsByUid, context, userContext, now }) {
  ...
  for (const bundle of bundles) {
    ...
    const targetMs = dateClockMs(bundle.lesson.date, bundle.lesson.startTime) - prefs.assessmentReminderMinutes * 60_000;
    if (!dueMs(targetMs, now)) continue;
...
async function dispatchPendingEvents({ root, messaging, subscriptionsByUid, usersByClass, contexts }) {
  ...
  for (const event of events) {
    ...
    await root.collection("notificationEvents").doc(event.id).set({
      status: "sent",
      sentAtMs: Date.now(),
...
export async function dispatchSchoolLifeNotifications({ db, messaging, now = new Date() }) {
  ...
  const subscriptionsByUid = lazySubscriptionMap(root);
  const userSnapshot = await root.collection("schoolLifeUsers").limit(3000).get();
  const users = userSnapshot.docs
    .map((document) => ({ uid: document.id, id: document.id, ...document.data() }))
    .filter((item) => item.classKey && item.enabled !== false);
  ...
  for (const classKey of classKeys) {
    const context = await loadClassContext(root, classKey, clock.date);
    contexts.set(classKey, context);
    const classUsers = users.filter((item) => item.classKey === classKey);
    const loaded = [];
    for (const user of classUsers) loaded.push(await loadUserContext(root, user, context));
  ...
  return {
    implementation: "bounded-read-v2",
    date: clock.date,
