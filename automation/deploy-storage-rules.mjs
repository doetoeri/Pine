...
const RELEASE_NAME = `projects/${PROJECT_ID}/releases/firebase.storage`;
...
async function api(token, path, options = {}, allow404 = false) {
...
async function main() {
  ...
  // Creating a ruleset also validates the Storage rules source.
  ...
  const existingRelease = await api(token, RELEASE_NAME, { method: "GET" }, true);
  ...
  if (existingRelease) {
    ...
    await api(token, `projects/${PROJECT_ID}/releases`, {
      method: "POST",
      body: JSON.stringify({
        name: RELEASE_NAME,
        rulesetName: ruleset.name,
  ...
  console.log(JSON.stringify({
    ...
    releaseMode: existingRelease ? "updated" : "created",
  }));
}
