const STYLE_ID = "pincon-group-share-v2-style";
let observedWorkspace = null;
let workspaceObserver = null;
let enhanceTimer = null;

function locked() {
  return Boolean(globalThis.__PINCON_TOUCH_STABILITY__?.locked?.());
}
...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .pincon-drive-item[data-pincon-share-enhanced="1"] .pincon-feature-actions{min-height:44px}
  `;
...
function cardPayload(card) {
  ...
  if (anchor) {
    return { kind: "link", title, url: anchor.href || anchor.textContent?.trim() || "", text: anchor.textContent?.trim() || anchor.href || "" };
  }
...
async function shareCard(card) {
  ...
  if (navigator.share) {
    try {
      await navigator.share(payload.kind === "link"
...
function enhanceWorkspaceNow() {
  ...
  if (linkButton && !linkButton.dataset.shareLabel) {
    linkButton.dataset.shareLabel = "1";
    linkButton.innerHTML = '<md-icon slot="icon">add_link</md-icon>링크 공유';
  }

  const noteButton = workspace.querySelector('[data-workspace-action="new-note"]');
  if (noteButton && !noteButton.dataset.shareLabel) {
    noteButton.dataset.shareLabel = "1";
    noteButton.innerHTML = '<md-icon slot="icon">text_snippet</md-icon>텍스트 공유';
  }

  workspace.querySelectorAll('[data-action="open-drive"]').forEach((button) => {
    if (!button.dataset.shareLabel) {
      button.dataset.shareLabel = "1";
  ...
  if (linkDialog) {
    const h = linkDialog.querySelector("h2");
    if (h) h.textContent = "링크 공유";
    const s = linkDialog.querySelector('button[type="submit"]');
    if (s) s.textContent = "공유";
  }

  const noteDialog = document.getElementById("pincon-drive-note-dialog");
  if (noteDialog) {
    const h = noteDialog.querySelector("h2");
    if (h) h.textContent = "텍스트 공유";
    const s = noteDialog.querySelector('button[type="submit"]');
    if (s) s.textContent = "공유";
  }
}
...
function scheduleEnhance(delay = 70) {
  ...
  enhanceTimer = setTimeout(enhanceWorkspaceNow, delay);
}
...
function attachWorkspaceObserver() {
  ...
  if (!workspace) return;
  workspaceObserver = new MutationObserver(() => scheduleEnhance(70));
...
document.addEventListener("click", async (event) => {
  ...
  const button = path.find((node) => node?.dataset?.shareAction) || event.target.closest?.("[data-share-action]");
  if (!button) return;
  const card = button.closest?.(".pincon-drive-item") || path.find((node) => node?.classList?.contains?.("pincon-drive-item"));
  if (!card) return;
