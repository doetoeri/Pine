...
function selectedClass() {
  ...
  return {
    grade: Number(profile.grade || kg || 1),
    classNumber: Number(profile.classNumber || kc || 8),
  };
}
...
function parseCsv(text) {
  ...
  const studentHeader = headers.includes("studentId")
    ? "studentId"
    : headers.includes("studentNumber")
      ? "studentNumber"
      : "";
  const classHeader = headers.includes("classNumber")
    ? "classNumber"
    : headers.includes("class")
      ? "class"
      : "";

  if (!studentHeader || !classHeader || !["name", "grade", "number"].every((h) => headers.includes(h))) {
    throw new Error("CSV 머리글은 studentNumber,name,grade,classNumber,number 형식이어야 합니다. class도 호환됩니다.");
  }
  ...
  return lines.slice(1).map((line) => {
    ...
    return {
      studentNumber: row[studentHeader],
      name: row.name,
      grade: Number(row.grade),
      classNumber: Number(row[classHeader]),
...
function classOptions(max, selected) {
...
function previewMarkup() {
  if (!preview) return "";
  const commitLabel = rosterCommitted
    ? `명단 ${preview.valid}명 등록 완료`
    : preview.conflicts
      ? "충돌 확인 후 등록"
      : `${preview.valid}명 등록`;
  return `<div class="enroll-summary"><div class="enroll-stat"><span>신규</span><strong>${preview.new}</strong></div><div class="enroll-stat"><span>기존/연결</span><strong>${preview.existing}</strong></div><div class="enroll-stat"><span>충돌 · 오류</span><strong>${preview.conflicts + preview.errors}</strong></div></div><div class="enroll-actions"><md-filled-button id="enrollCommit" ${preview.errors || rosterCommitted ? "disabled" : ""}>${commitLabel}</md-filled-button></div>`;
}
...
async function refreshStatus(render = true) {
  ...
  try {
    ...
    if (statusData.session?.active && Number(statusData.session.expiresAtMs || 0) > Number(statusData.serverNowMs || Date.now())) {
      pollTimer = setTimeout(() => refreshStatus(true), 1800);
    }
  } catch (error) {
...
async function commitRoster({ askConflicts = true } = {}) {
  if (!preview || !rosterRows.length) throw new Error("먼저 학생 명단 CSV를 선택하세요.");
  if (preview.errors) throw new Error(`CSV 오류 ${preview.errors}건을 먼저 수정하세요.`);
  if (preview.conflicts && askConflicts) {
    const ok = confirm(`기존 정보와 다른 학생 ${preview.conflicts}명이 있습니다. 기존 활성화 상태와 UID는 보존하고 명단 정보만 갱신할까요?`);
    if (!ok) throw new Error("명단 등록을 취소했습니다.");
  }
...
function bind() {
  ...
  section.querySelector("#enrollCsv")?.addEventListener("change", async (event) => {
    try {
      ...
      renderSection(true);
      message(`${preview.valid}명의 명단을 확인했습니다. 가입 시작 시 자동 등록됩니다.`);
    } catch (error) {
      rosterRows = [];
      preview = null;
      rosterCommitted = false;
  ...
  section.querySelector("#enrollCommit")?.addEventListener("click", async () => {
    try {
      await commitRoster({ askConflicts: true });
      renderSection(true);
      message(`${preview.valid}명의 학생 명단을 등록했습니다.`);
    } catch (error) {
      message(error.message, true);
    }
  });
  ...
  section.querySelector("#startEnrollment")?.addEventListener("click", async () => {
    try {
      const grade = Number(section.querySelector("#enrollGrade").value);
      const classNumber = Number(section.querySelector("#enrollClass").value);
      const durationMinutes = Number(section.querySelector("#enrollDuration").value);

      if (rosterRows.length && preview && !rosterCommitted) {
        await commitRoster({ askConflicts: true });
      }
      ...
      if (Number(statusData.total || 0) < 1) {
        await api({ action: "session-end", sessionId }).catch(() => {});
        activeSession = null;
        statusData = null;
        clearTimeout(pollTimer);
        renderSection(true);
        throw new Error("등록된 학생이 0명이라 가입 세션을 시작하지 않았습니다. CSV를 선택한 뒤 다시 시도하세요.");
      }

      renderSection(true);
      message(`${statusData.total}명 명단으로 가입 세션을 시작했습니다.`);
      clearTimeout(pollTimer);
      pollTimer = setTimeout(() => refreshStatus(true), 1800);
    } catch (error) {
      message(error.message, true);
  ...
  section.querySelector("#copyJoin")?.addEventListener("click", async () => {
    const s = activeSession.session || activeSession;
    const url = activeSession.joinUrl || `https://pincon.app/next/join/?session=${s.id}`;
    await navigator.clipboard.writeText(url);
    message("가입 링크를 복사했습니다.");
  });

  section.querySelector("#extendJoin")?.addEventListener("click", async () => {
    const s = activeSession.session || activeSession;
    statusData = await api({ action: "session-extend", sessionId: s.id, minutes: 5 });
    renderSection(true);
  });

  section.querySelector("#endJoin")?.addEventListener("click", async () => {
    if (!confirm("현재 학급 가입 세션을 종료할까요?")) return;
    const s = activeSession.session || activeSession;
    statusData = await api({ action: "session-end", sessionId: s.id });
    clearTimeout(pollTimer);
    renderSection(true);
    message("가입 세션을 종료했습니다.");
  });

  section.querySelectorAll("[data-filter]").forEach((el) => {
  ...
  section.querySelectorAll("[data-student-action]").forEach((el) => {
    el.addEventListener("click", async () => {
      const studentNumber = el.dataset.student;
      const studentAction = el.dataset.studentAction;
      ...
      try {
        ...
        if (result?.session) {
          ...
          statusData = null;
          message("개별 가입 링크를 열었습니다.");
          await refreshStatus(true);
        } else {
          await refreshStatus(true);
          message("학생 계정 상태를 변경했습니다.");
        }
      } catch (error) {
        message(error.message, true);
      }
