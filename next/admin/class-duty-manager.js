...
function activeMembers() {
  return (view?.members || []).filter((item) => item.status === "ACTIVE").sort((a, b) => Number(a.number || 0) - Number(b.number || 0));
}

function activeDepartments() {
  return (view?.departments || []).filter((item) => item.active !== false);
}

function activeRoles() {
  return (view?.onePersonRoles || []).filter((item) => item.active !== false);
}

function departmentName(id) {
  return (view?.departments || []).find((item) => item.id === id)?.name || "미배정";
}

function memberLabel(member) {
  return `${Number(member.number) || "-"}번 ${member.name || "이름 없음"}`;
}

function roleCards() {
  const roles = activeRoles();
  if (!roles.length) return `<div class="admin-empty"><md-icon>assignment_ind</md-icon><strong>먼저 1인1역을 만들어주세요</strong><span>아래 학급 운영 설정에서 역할 이름과 시점을 등록할 수 있습니다.</span></div>`;
  return `<div class="pincon-duty-role-grid">${roles.map((role) => {
    ...
    return `<article class="pincon-duty-role" data-assigned="${holder ? "true" : "false"}">
      ...
      <md-filled-tonal-button data-assign-role="${escapeHtml(role.id)}">${holder ? "변경" : "배정"}</md-filled-tonal-button>
    </article>`;
  }).join("")}</div>`;
}
...
function departmentCards() {
  ...
  if (!departments.length) return `<div class="admin-empty"><md-icon>groups</md-icon><strong>청소 부서가 없습니다</strong><span>학급자치회 부서를 먼저 등록해주세요.</span></div>`;
  return `<div class="pincon-duty-department-grid">${departments.map((department) => {
    ...
    const names = members.slice(0, 8).map((member) => `<span>${escapeHtml(memberLabel(member))}</span>`).join("");
    return `<article class="pincon-duty-department">
      ...
      <div class="pincon-duty-department__members">${members.length ? names : `<span class="is-empty">배정된 학생이 없습니다</span>`}${members.length > 8 ? `<span>+${members.length - 8}명</span>` : ""}</div>
      <md-outlined-button data-manage-department="${escapeHtml(department.id)}"><md-icon slot="icon">group</md-icon>구성원 관리</md-outlined-button>
    </article>`;
  }).join("")}</div>`;
}
...
function cleaningAssignmentMarkup() {
  ...
  if (!assignment) return `<div class="pincon-cleaning-current is-empty"><div><span>오늘의 대걸레 당번</span><strong>아직 배정되지 않음</strong><small>${escapeHtml(cleaning.department?.name || "")}</small></div><md-icon>person_add</md-icon></div>`;
  return `<div class="pincon-cleaning-current" data-status="${escapeHtml(assignment.status || "ASSIGNED")}">
    <div><span>오늘의 대걸레 당번</span><strong>${escapeHtml(`${Number(assignment.assigneeNumber) || "-"}번 ${assignment.assigneeName || "학생"}`)}</strong><small>${escapeHtml(assignment.selectionReason || "배정 사유 없음")}</small></div>
    <em>${escapeHtml(STATUS_LABELS[assignment.status] || assignment.status || "배정")}</em>
  </div>`;
}

function cleaningStatsMarkup() {
  const rows = view?.cleaning?.memberStats || [];
  if (!rows.length) return `<span class="pincon-duty-muted">이 부서에 배정된 학생이 없습니다.</span>`;
  return `<div class="pincon-cleaning-stats">${rows.map((row) => `<span><strong>${escapeHtml(`${row.number}번 ${row.name}`)}</strong><em>${row.count}회</em></span>`).join("")}</div>`;
}
...
function recommendationMarkup() {
  ...
  return `<div class="pincon-cleaning-recommendation"><md-icon>auto_awesome</md-icon><div><span>공정 추천</span><strong>${escapeHtml(memberLabel(recommendation.candidate))}</strong><small>${escapeHtml(recommendation.reason || "")}</small></div><div class="pincon-cleaning-recommendation__facts"><span>이번 달 ${Number(recommendation.fairness?.monthlyCount || 0)}회</span><span>${recommendation.fairness?.lastAssignedDate ? `마지막 ${escapeHtml(recommendation.fairness.lastAssignedDate)}` : "이번 달 첫 배정"}</span></div><md-filled-button data-confirm-recommendation>이 학생으로 배정</md-filled-button></div>`;
}
...
function cleaningMarkup() {
  ...
  const pendingCount = cleaning?.pendingRequests?.length || 0;
  return `<div class="pincon-cleaning-manager">
    ...
    ${pendingCount ? `<div class="pincon-cleaning-pending"><md-icon>pending_actions</md-icon><span>이 부서에 처리 대기 중인 교환·면제 요청이 ${pendingCount}건 있습니다.</span></div>` : ""}
  </div>`;
}
...
function markup() {
  ...
  return `<section class="admin-card admin-card--wide pincon-duty-manager" id="pinconDutyManager" aria-labelledby="pincon-duty-title">
    ...
    <div id="pinconDutyStatus" class="pincon-duty-status" role="status" aria-live="polite">${loading ? "학급 운영 정보를 불러오는 중…" : ""}</div>
    <section class="pincon-duty-section"><div class="pincon-duty-section__head"><div><span>01</span><div><h3>1인1역 배정</h3><p>한 학생에게 한 역할만 배정하며, 같은 역할을 다른 학생에게 옮기면 기존 배정은 자동 해제됩니다.</p></div></div></div>${roleCards()}</section>
    <section class="pincon-duty-section"><div class="pincon-duty-section__head"><div><span>02</span><div><h3>청소 부서 구성</h3><p>부서별 구성원을 정하면 해당 부장과 학급 운영자가 그 안에서 당번을 배정할 수 있습니다.</p></div></div></div>${departmentCards()}</section>
    <section class="pincon-duty-section"><div class="pincon-duty-section__head"><div><span>03</span><div><h3>청소당번 관리</h3><p>대걸레 당번을 추천·자동·직접 배정하고 수행 완료까지 기록합니다.</p></div></div></div>${cleaningMarkup()}</section>
  </section>`;
}
...
async function post(action, payload = {}) {
  return accountRequest("/api/class-ops/duties", {
    method: "POST",
    body: { action, classKey: view.classKey, ...payload },
  });
}
...
function openRoleDialog(role) {
  ...
  dialog.querySelector("[data-dialog-save]")?.addEventListener("click", async () => {
    ...
    if (!uid && !current) { dialog.close?.(); return; }
    status.textContent = "배정 저장 중…";
    try {
      await post("ASSIGN_ONE_PERSON_ROLE", { userUid: uid || current.uid, roleId: uid ? role.id : "" });
      recommendation = null;
      await load(true);
      dialog.close?.();
    } catch (error) {
      status.textContent = error?.message === "one-person-role-not-found" ? "역할 정보를 찾지 못했습니다." : "배정을 저장하지 못했습니다.";
    }
  });
}
...
function openDepartmentDialog(department) {
  ...
  const rows = activeMembers().map((member) => `<label class="pincon-duty-member-check"><md-checkbox data-duty-member="${escapeHtml(member.uid)}" ${current.has(member.uid) ? "checked" : ""}></md-checkbox><span><strong>${escapeHtml(memberLabel(member))}</strong><small>${escapeHtml(member.departmentId && member.departmentId !== department.id ? `현재 ${departmentName(member.departmentId)}` : member.departmentId === department.id ? "현재 이 부서" : "부서 미배정")}</small></span></label>`).join("");
  const dialog = makeDialog(`${department.name} 구성원`, `<div class="pincon-duty-dialog__intro"><md-icon>groups</md-icon><div><strong>${escapeHtml(department.name)}</strong><span>체크한 학생이 이 부서의 청소 후보가 됩니다. 다른 부서 학생을 체크하면 이 부서로 이동합니다.</span></div></div><div class="pincon-duty-member-list">${rows}</div>`, "구성 저장");
  dialog.querySelector("[data-dialog-save]")?.addEventListener("click", async () => {
    const status = dialog.querySelector("[data-dialog-status]");
    const userUids = [...dialog.querySelectorAll("[data-duty-member]")].filter((box) => box.checked).map((box) => box.dataset.dutyMember);
    status.textContent = "구성원을 저장하는 중…";
    try {
      await post("SET_DEPARTMENT_MEMBERS", { departmentId: department.id, userUids });
...
function openManualCleaningDialog() {
  ...
  dialog.querySelector("[data-dialog-save]")?.addEventListener("click", async () => {
    ...
    try {
      await post("CLEANING_ASSIGN", { departmentId: selectedDepartmentId, date: selectedDate, assigneeUid });
      recommendation = null;
      await load(true);
      dialog.close?.();
    } catch {
...
async function runAction(button, action, payload, pendingText) {
  const section = root.querySelector("#pinconDutyManager");
  ...
  try {
    ...
    if (status) status.textContent = `처리하지 못했습니다. ${error?.message || "다시 시도해주세요."}`;
    return null;
  } finally {
...
function bind(section) {
  section.querySelectorAll("[data-assign-role]").forEach((button) => button.addEventListener("click", () => {
    const role = activeRoles().find((item) => item.id === button.dataset.assignRole);
    if (role) openRoleDialog(role);
  }));
  section.querySelectorAll("[data-manage-department]").forEach((button) => button.addEventListener("click", () => {
    const department = activeDepartments().find((item) => item.id === button.dataset.manageDepartment);
  ...
  section.querySelector("#pinconCleaningRefresh")?.addEventListener("click", () => load(true));
  ...
  section.querySelector("#pinconCleaningRecommend")?.addEventListener("click", async (event) => {
    ...
    try {
      const result = await post("CLEANING_RECOMMEND", { departmentId: selectedDepartmentId, date: selectedDate });
      recommendation = { ...result, departmentId: selectedDepartmentId };
      status.textContent = "추천 후보를 계산했습니다.";
      mount({ force: true });
    } catch (error) {
  ...
  section.querySelector("[data-confirm-recommendation]")?.addEventListener("click", async (event) => {
    const uid = recommendation?.candidate?.uid;
    if (!uid) return;
    await runAction(event.currentTarget, "CLEANING_ASSIGN", { departmentId: selectedDepartmentId, date: selectedDate, assigneeUid: uid }, "추천 학생으로 배정하는 중…");
  });
  section.querySelector("#pinconCleaningAuto")?.addEventListener("click", (event) => runAction(event.currentTarget, "CLEANING_AUTO_ASSIGN", { departmentId: selectedDepartmentId, date: selectedDate }, "공정 기준으로 자동 배정하는 중…"));
  section.querySelector("#pinconCleaningComplete")?.addEventListener("click", (event) => runAction(event.currentTarget, "CLEANING_COMPLETE", { departmentId: selectedDepartmentId, date: selectedDate }, "청소 완료로 기록하는 중…"));
  section.querySelector("#pinconCleaningClear")?.addEventListener("click", (event) => runAction(event.currentTarget, "CLEANING_CLEAR", { departmentId: selectedDepartmentId, date: selectedDate }, "배정을 취소하는 중…"));
}
...
async function load(force = false) {
  ...
  loading = true;
  try {
    const params = new URLSearchParams({ classKey, date: selectedDate });
    if (selectedDepartmentId) params.set("departmentId", selectedDepartmentId);
    view = await accountRequest(`/api/class-ops/duties?${params.toString()}`);
