...
function installStyles() {
  if (document.querySelector("#pinconAssessmentV2Styles")) return;
  const style = document.createElement("style");
  style.id = "pinconAssessmentV2Styles";
  style.textContent = `
    .pincon-assessment-v2{margin-top:8px;display:grid;gap:14px}
    .pincon-assessment-v2-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;padding:16px;border:1px solid var(--md-sys-color-outline-variant,#d9ddd7);border-radius:20px;background:var(--md-sys-color-surface-container-low,#f7f8f5)}
    .pincon-assessment-v2-head h3{margin:0 0 5px;font-size:18px}.pincon-assessment-v2-head p{margin:0;color:var(--md-sys-color-on-surface-variant,#636863);font-size:13px;line-height:1.55}
    ...
    @media(max-width:760px){.pincon-assessment-v2-head{flex-direction:column}.pincon-assessment-grid{grid-template-columns:repeat(5,minmax(105px,1fr))}.pincon-assessment-guide-row{grid-template-columns:1fr}.pincon-assessment-status{width:100%;margin-left:0}}
  `;
...
function targetOfDesk(uid) {
  const index = draftOwners.findIndex((value) => value === uid);
  return index >= 0 ? `수행평가 자리 ${index + 1}` : "목적지 미정";
}

function seatTarget(uid) {
  const index = (plan().seatOrder || []).findIndex((value) => value === uid);
...
function render() {
  ...
  hostPanel.querySelectorAll(":scope > .pincon-layout-toolbar, :scope > .pincon-layout-actions, :scope > .pincon-layout-classroom").forEach((node) => node.classList.add("pincon-assessment-legacy-hidden"));
  hostPanel.querySelector("[data-assessment-v2]")?.remove();
  ...
  section.className = "pincon-assessment-v2";
  section.dataset.assessmentV2 = "";
  section.innerHTML = `
    <div class="pincon-assessment-v2-head"><div><h3>5줄 수행평가 이동 설계</h3><p>학생은 번호순으로 앉습니다. 책상은 원하는 위치로 옮길 수 있으며 책상 주인과 실제 착석 학생은 서로 달라도 됩니다.</p></div><span class="pincon-assessment-badge">5줄 · 번호순 고정</span></div>
