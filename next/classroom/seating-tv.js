...
function render() {
  ...
  root.innerHTML = `<div class="tv-shell"><header class="tv-header"><div><div class="tv-heading"><span class="wordmark">PinCon</span><h1>${esc(view.classKey)} 우리 반 자리표</h1></div><p>${demo ? `<span class="tv-demo">예시 명단 · 실제 배치 아님</span>` : "3분단 · 6줄 / 6줄 / 5줄 · 2명씩 짝"}</p></div><div class="tv-controls"><button id="flip">교탁을 ${flipped ? "위로" : "아래로"}</button><button id="fullscreen">${document.fullscreenElement ? "전체화면 종료" : "전체화면"}</button><button id="print">인쇄</button></div></header>
    ${hasSeats ? `<div class="tv-room-wrap ${flipped ? "is-flipped" : ""}"><div class="seat-board">칠판 · 교탁</div><div class="section-labels">${(flipped ? [3, 2, 1] : [1, 2, 3]).map(n => `<span>${n}분단</span>`).join("")}</div><div class="planner-room">${indexes.map(index => {
      ...
      return `<div class="desk ${blocked || !s ? "is-blocked" : ""}"><small>${Math.floor(index / 6) + 1}–${index % 6 + 1}</small><strong>${esc(blocked ? "―" : s?.name || "빈자리")}</strong><span class="desk-number">${s && !blocked ? `${s.number}번` : ""}</span></div>`;
...
document.addEventListener("fullscreenchange", () => { if (!document.fullscreenElement) { wakeLock?.release().catch(() => {}); wakeLock = null; } const button = document.getElementById("fullscreen"); if (button) button.textContent = document.fullscreenElement ? "전체화면 종료" : "전체화면"; });
document.addEventListener("visibilitychange", () => { clearInterval(timer); if (document.visibilityState === "visible") { load(); keepAwake(); timer = setInterval(load, 15000); } });
