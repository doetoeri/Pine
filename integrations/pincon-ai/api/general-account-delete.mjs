...
function validClassKey(value) {
  return /^[1-3]-(?:[1-9]|10)$/.test(String(value || ""));
}

function isPrivileged(profile) {
  return Array.isArray(profile?.roles) && profile.roles.some((role) => PRIVILEGED.has(role));
}

function isPrivilegedLegacy(role) {
...
async function deleteCandidate(actor, profile, manifestRef, now) {
  ...
  batch.set(registrationRef(profile.studentNumber), {
    claimStatus: "READY",
...
async function repairGhosts(actor, classKey, manifestRef) {
  ...
  for (const doc of snap.docs) {
    ...
    try {
      ...
      if (current) {
        const patch = {
          activated: current.status !== "DISABLED",
          uid: current.uid,
          existingUid: current.uid,
          accountStatus: current.status || "ACTIVE",
          claimState: current.status === "DISABLED" ? "READY" : "CLAIMED",
          claimNonce: "",
          claimAttemptId: "",
          claimLeaseUntilMs: 0,
          updatedAtMs: Date.now(),
        ...
        await studentRef(studentNumber).set(patch, { merge: true });
        relinked += 1;
        continue;
      }
      ...
      await manifestRef.collection("roster").doc(studentNumber).set({
        before: {
          ...
          existingUid: String(student.existingUid || ""),
          accountStatus: String(student.accountStatus || ""),
        },
      ...
      for (const uid of orphanUids) {
        ...
        if (legacySnap?.exists && legacySnap.data()?.managedByAccountSystem === true) {
          await roleRef(uid).delete().catch(() => {});
        }
      }
...
export default async function generalAccountDelete(req, res) {
  ...
  try {
    ...
    if (!hasRole(actor, ROLE.ADMIN) && actor.classKey !== classKey) {
      throw Object.assign(new Error("class-scope-denied"), { status: 403 });
    }

    const requested = [...new Set((Array.isArray(body.studentNumbers) ? body.studentNumbers : [])
    ...
    for (const studentNumber of requested) {
      try {
        ...
        if (profile.uid === actor.uid || isPrivileged(profile)) {
          preserved.push(studentNumber);
          continue;
        }
    ...
    await manifestRef.set({
      ...
      relinkedCount: repair.relinked,
      preservedCount: preserved.length + repair.preserved,
    ...
    await appendAccountAudit({
      actor,
      action: "GENERAL_ACCOUNTS_DELETE_AND_ENROLLMENT_REPAIR",
      targetUid: actor.uid,
      metadata: {
        classKey,
        backupId,
        ...
        repaired: String(repair.repaired),
        relinked: String(repair.relinked),
        preserved: String(preserved.length + repair.preserved),
    ...
    return sendJson(res, 200, {
      ...
      preserved: preserved.length + repair.preserved,
      failed,
    }, headers);
  } catch (error) {
    return sendJson(res, Number(error?.status || 500), {
      error: Number(error?.status || 500) >= 500 ? "general-account-delete-failed" : String(error?.message || "general-account-delete-failed"),
    }, headers);
  }
}
