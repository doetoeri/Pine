/* WORD MASTER Glass Recall - test handwriting adapter v3
 * The legacy OCR canvas is removed from the DOM, not hidden.
 * Tests use the exact same WMHandwriting InkEngine as learning.
 * OCR image routing is handled by ocr-canvas-bridge.js.
 */
(function () {
  "use strict";
  ...
  var STYLE_ID = "wm-write-study-test-style-v5";
  var active = null;
  var scanTimer = null;
  ...
  function addStyles() {
    ...
    style.textContent =
      ...
      "[data-wm-retired-ocr-wrap='true']{display:none!important}" +
      "@media (prefers-reduced-motion:reduce){#" + PANEL_ID + " *{transition:none!important;animation:none!important}}";
    (document.head || document.documentElement).appendChild(style);
  }

  /* S Pen side-button state, shared with both learning and test InkEngine instances. */
  var Pen = {
    barrelUntil: 0,
    eraserUntil: 0,
    lastPenAt: 0,
  ...
  /* Patch the shared InkEngine once, so learning and test keep identical pen behavior. */
  ...
  if (proto && !proto.__wmReusePenPatchedV3) {
    proto.__wmReusePenPatchedV3 = true;
  ...
  function isOurCanvas(canvas) {
    if (!canvas) return false;
    if (active && canvas === active.canvas) return true;
    if (contains(document.getElementById(PANEL_ID), canvas)) return true;
    if (contains(document.getElementById("wm-write-study-panel-v2"), canvas)) return true;
    return false;
  }
  ...
  function nativeCandidates() {
    ...
    var list = root.querySelectorAll("canvas"), out = [], i, c, r, area;
    for (i = 0; i < list.length; i += 1) {
      c = list[i];
      if (isOurCanvas(c) || !visible(c)) continue;
      try { r = c.getBoundingClientRect(); } catch (ignore) { r = { width: 0, height: 0 }; }
      area = r.width * r.height;
      if (r.width < 120 || r.height < 70) continue;
      out.push({ canvas: c, area: area });
    }
    out.sort(function (a, b) { return b.area - a.area; });
    return out;
  }

  function retireNativeCanvas(canvas) {
    if (!canvas || isOurCanvas(canvas)) return;
    var parent = canvas.parentNode;
    try { parent.removeChild(canvas); } catch (ignore) {
      try { canvas.remove(); } catch (ignore2) {}
    }
    if (parent && parent.nodeType === 1 && !parent.children.length && !textOf(parent)) {
      try { parent.setAttribute("data-wm-retired-ocr-wrap", "true"); } catch (ignore3) {}
    }
  }

  function purgeNativeCanvases() {
    var items = nativeCandidates(), i;
    for (i = 0; i < items.length; i += 1) retireNativeCanvas(items[i].canvas);
  }
  ...
  function createPanel(nativeCanvas, grade) {
    ...
    help.textContent = "학습할 때와 같은 필기 엔진입니다. 이 캔버스 자체가 OCR 입력입니다.";
    ...
    /* Delete the legacy OCR canvas only after the replacement is mounted. */
    retireNativeCanvas(nativeCanvas);
    return result;
  }
  ...
  function destroyActive() {
    ...
    try { if (active.panel && active.panel.parentNode) active.panel.parentNode.removeChild(active.panel); } catch (ignore2) {}
    active = null;
  }
  ...
  function sync() {
    ...
    if (active) {
      ...
      if (!document.documentElement.contains(active.panel) && grade.parentNode) {
        grade.parentNode.insertBefore(active.panel, grade);
      }
      clearForQuestion(questionKey(grade));
      /* React may recreate its old OCR canvas on rerender. Delete it again. */
      purgeNativeCanvases();
      return;
    }
    ...
    active = createPanel(candidates[0].canvas, grade);
    purgeNativeCanvases();
  }

  function schedule() {
    if (scanTimer) return;
    scanTimer = window.setTimeout(sync, 120);
  }
